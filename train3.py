...
def main(l_rate, gamma, stop_layer, n_epochs, k, batch_size,save_path, load_path, seed):
    ...
    for fold, (train_idx,val_idx) in enumerate(splits.split(np.arange(len(train_set)))):
        ...
        # save layer2 output
        model.layer2.register_forward_hook(get_activation(stop_layer))
        ...
        for images, labels in train_loader:
            ...
            output_train_layer2.append(activation[stop_layer].flatten(1,3))
        ...
        svm = LinearSVC()
        svm.fit(output_train_layer2, train_labels)
        ...
        for images, labels in valid_loader:
            ...
            outputs = model(images)
            val_accuracy += svm.score(activation[stop_layer].flatten(1,3), labels)

        val_accuracy/=len(valid_loader)

        if best_accuracy < val_accuracy:
            pickle.dump(svm, open(save_path, 'wb'))
    ...
    model.load_state_dict(torch.load(load_path))
    model.layer2.register_forward_hook(get_activation(stop_layer))
    svm = pickle.load(open(save_path, 'rb'))
